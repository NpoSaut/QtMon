...
void StateSerializer::Deserialize(ViewModels::SystemStateViewModel *model, const QString stateLine)
{
    ...
    model->setLight(parameters[21].toInt());
    model->setAlsnFreqTarget(parameters[22].toInt());
    model->setAlsnFreqFact(parameters[23].toInt());
    model->setAutolockTypeTarget(parameters[24].toInt());
    model->setAutolockTypeFact(parameters[25].toInt());
    model->setTime(parameters[26]);
    model->setDate(parameters[27]);
    model->setIsRegistrationTapeActive(parameters[28].toInt());
    model->setDriveModeTarget(parameters[29].toInt());
    model->setDriveModeFact(parameters[30].toInt());
    model->setIronWheels(parameters[31].toInt());
    model->setIsVigilanceRequired(parameters[32].toInt());
    model->setIsTractionOn(parameters[33].toInt());
    model->setDirection(parameters[34].toInt());
    model->setOrdinate(parameters[35].toInt());
    model->setNextStatinName(parameters[36]);
    model->setNextTargetKind(parameters[37].toInt());
    model->setNextTargetName(parameters[38]);
    model->setNextTargetDistance(parameters[39].toInt());
    model->setSautTargetDistance(parameters[40].toInt());
    model->setNotificationText(parameters[41]);
    model->setTrackNumber(parameters[42].toInt());
    model->setMachinistNumber(parameters[43].toInt());
    model->setTrainNumber(parameters[44].toInt());
    model->setWagonCount(parameters[45].toInt());
    model->setAxlesCount(parameters[46].toInt());
    model->setTrainMass(parameters[47].toInt());
    model->setManualOrdinateEnable(parameters[48].toInt());
    model->setManualOrdinate(parameters[49].toInt());
    model->setManualOrdinateIncreaseDirection(parameters[50].toInt());
    model->setAutolockSpeed(parameters[51].toInt());
    model->setTsvcIsOnline(parameters[52].toInt());
    model->setTsvcIsMachinistCheerful(parameters[53].toInt());
    model->setTsvcIsVigilanceRequired(parameters[54].toInt());
    model->setTsvcIsPreAlarmActive(parameters[55].toInt());
